...
- special expressions: some information of the Remote can be logged:
  - `${Remote.tStates}` - the number of T-States since beginning of the debug session (only available in 'zsim')
  -  `${Remote.cpuFrequency}` - the frequency in Hz, e.g. 3500000 (only available in 'zsim')
...
- examples:
    ...
    - `LOGPOINT Freq=${Remote.cpuFrequency/1000000}MHz`
...
E.g. on every step-over you get an info about the used ∆T-States and time. This can be used to measure the performance of sub routines. The ∆T-States are the T-States since last break. (Note: the calculation of the time uses the CPU frequency, if the CPU frequency changes during execution the calculated time will be wrong.)
